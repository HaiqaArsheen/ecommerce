...
router.post('/submit-westerntopsform', (req, res) => {
  ...
  const uploadDir = path.join(__dirname, '../img/western');
  ...
  const dbImage1Path = `../img/western/${path.basename(image1Path)}`;
  const dbImage2Path = `../img/western/${path.basename(image2Path)}`;
  const dbImage3Path = `../img/western/${path.basename(image3Path)}`;

  // Insert data into the database
  const query = `INSERT INTO westerntop (name, price, description, image1, image2, image3) VALUES (?, ?, ?, ?, ?, ?)`;
...
router.get('/', (req, res) => {
  const sql = 'SELECT * FROM  westerntop';
...
router.post('/update-product/:id', (req, res) => {
  ...
  let updateQuery = 'UPDATE  westerntop  SET name = ?, price = ?, description = ?';
...
router.delete('/product/:id', (req, res) => {
  const productId = req.params.id;
  const query = 'DELETE FROM  westerntop  WHERE id = ?';
  db.query(query, [productId], (err, results) => {
    if (err) {
