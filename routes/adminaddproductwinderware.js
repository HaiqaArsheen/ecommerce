...
router.post('/submit-winderwaresform', (req, res) => {
  ...
  const uploadDir = path.join(__dirname, '../img/winterware');
  ...
  const dbImage1Path = `../img/winterware/${path.basename(image1Path)}`;
  const dbImage2Path = `../img/winterware/${path.basename(image2Path)}`;
  const dbImage3Path = `../img/winterware/${path.basename(image3Path)}`;

  // Insert data into the database
  const query = `INSERT INTO winderwares (name, price, description, image1, image2, image3) VALUES (?, ?, ?, ?, ?, ?)`;
...
router.get('/', (req, res) => {
  const sql = 'SELECT * FROM  winderwares';
...
router.post('/update-product/:id', (req, res) => {
  ...
  let updateQuery = 'UPDATE  winderwares  SET name = ?, price = ?, description = ?';
  const updateValues = [name, price, description];

  if (image1) {
    const image1Path = path.join(__dirname, '../img/jeans', `${Date.now()}_1.webp`);
    image1.mv(image1Path, (err) => {
      if (err) {
        console.error('Error uploading image1:', err);
        return res.status(500).send('Error uploading image1.');
    ...
    const dbImage1Path = `../img/jeans/${path.basename(image1Path)}`;
    updateQuery += ', image1 = ?';
    updateValues.push(dbImage1Path);
  }

  if (image2) {
    const image2Path = path.join(__dirname, '../img/jeans', `${Date.now()}_2.webp`);
    image2.mv(image2Path, (err) => {
      if (err) {
        console.error('Error uploading image2:', err);
        return res.status(500).send('Error uploading image2.');
    ...
    const dbImage2Path = `../img/jeans/${path.basename(image2Path)}`;
    updateQuery += ', image2 = ?';
    updateValues.push(dbImage2Path);
  }
  if (image3) {
    const image3Path = path.join(__dirname, '../img/jeans', `${Date.now()}_3.webp`);
    image3.mv(image3Path, (err) => {
      if (err) {
        console.error('Error uploading image3:', err);
        return res.status(500).send('Error uploading image3.');
    ...
    const dbImage3Path = `../img/jeans/${path.basename(image3Path)}`;
    updateQuery += ', image3 = ?';
...
router.delete('/product/:id', (req, res) => {
  const productId = req.params.id;
  const query = 'DELETE FROM  winderwares  WHERE id = ?';
  db.query(query, [productId], (err, results) => {
    if (err) {
