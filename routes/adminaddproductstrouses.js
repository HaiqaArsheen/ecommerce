...
router.post('/submit-trousesform', (req, res) => {
  ...
  const uploadDir = path.join(__dirname, '../img/trouser');
  ...
  const dbImage1Path = `../img/trouser/${path.basename(image1Path)}`;
  const dbImage2Path = `../img/trouser/${path.basename(image2Path)}`;
  const dbImage3Path = `../img/trouser/${path.basename(image3Path)}`;

  // Insert data into the database
  const query = `INSERT INTO trouses (name, price, description, image1, image2, image3) VALUES (?, ?, ?, ?, ?, ?)`;
  console.log(query);
...
router.post('/update-product/:id', (req, res) => {
  ...
  let updateQuery = 'UPDATE  trouses SET name = ?, price = ?, description = ?';
...
router.delete('/product/:id', (req, res) => {
  const productId = req.params.id;
  const query = 'DELETE FROM  trouses WHERE id = ?';
  db.query(query, [productId], (err, results) => {
    if (err) {
