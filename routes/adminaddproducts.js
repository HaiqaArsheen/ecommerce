...
router.post('/submit-form', (req, res) => {
  ...
  const uploadDir = path.join(__dirname, '../img/bags');
  ...
  const dbImage1Path = `../img/bags/${path.basename(image1Path)}`;
  const dbImage2Path = `../img/bags/${path.basename(image2Path)}`;
  const dbImage3Path = `../img/bags/${path.basename(image3Path)}`;

  // Insert data into the database
  const query = `INSERT INTO products (name, price, description, image1, image2, image3) VALUES (?, ?, ?, ?, ?, ?)`;
...
router.post('/update-product/:id', (req, res) => {
  ...
  let updateQuery = 'UPDATE products SET name = ?, price = ?, description = ?';
  const updateValues = [name, price, description];

  if (image1) {
    const image1Path = path.join(__dirname, '../img/bags', `${Date.now()}_1.webp`);
    image1.mv(image1Path, (err) => {
      if (err) {
        console.error('Error uploading image1:', err);
        return res.status(500).send('Error uploading image1.');
  ...
  if (image2) {
    const image2Path = path.join(__dirname, '../img/bags', `${Date.now()}_2.webp`);
    image2.mv(image2Path, (err) => {
      if (err) {
        console.error('Error uploading image2:', err);
        return res.status(500).send('Error uploading image2.');
  ...
  if (image3) {
    const image3Path = path.join(__dirname, '../img/bags', `${Date.now()}_3.webp`);
    image3.mv(image3Path, (err) => {
      if (err) {
        console.error('Error uploading image3:', err);
        return res.status(500).send('Error uploading image3.');
