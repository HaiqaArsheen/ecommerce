...
router.post('/submit-tunicsform', (req, res) => {
  ...
  const uploadDir = path.join(__dirname, '../img/tunics');
  ...
  const dbImage1Path = `../img/tunics/${path.basename(image1Path)}`;
  const dbImage2Path = `../img/tunics/${path.basename(image2Path)}`;
  const dbImage3Path = `../img/tunics/${path.basename(image3Path)}`;

  // Insert data into the database
  const query = `INSERT INTO tunics (name, price, description, image1, image2, image3) VALUES (?, ?, ?, ?, ?, ?)`;
...
router.post('/update-product/:id', (req, res) => {
  ...
  let updateQuery = 'UPDATE  tunics  SET name = ?, price = ?, description = ?';
...
router.delete('/product/:id', (req, res) => {
  const productId = req.params.id;
  const query = 'DELETE FROM  tunics  WHERE id = ?';
  db.query(query, [productId], (err, results) => {
    if (err) {
